...
import { Settings, Save, Lock, AlertCircle, HardDrive, CheckCircle2, Cloud } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onSave }) => {
  ...
  const handleSave = () => {
    onSave({
      ...
      googleDriveEmail: isDriveConnected ? 'user@example.com' : undefined // Mock email
    });
    onClose();
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="block w-full pl-10 pr-3 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm"
              />
            </div>
          ...
          <div className="space-y-4">
            ...
            {isDriveConnected ? (
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm">
                    ...
                    <svg viewBox="0 0 87.3 78" className="w-6 h-6">
                      ...
                      <path d="M43.65 25l13.75-23.8c-1.35-.8-2.9-1.2-4.4-1.2h-18.5c-1.55 0-3.1.4-4.45 1.2l-13.75 23.8z" fill="#00AC47"/>
                      <path d="M73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 2.25-3.9c.8-1.4 1.2-2.95 1.2-4.5h-27.5l13.75 23.8c1.55 0 3.1-.4 4.45-1.2z" fill="#EA4335"/>
                      <path d="M43.65 25L29.9 1.2c-1.35.8-2.5 1.9-3.3 3.3l-20 34.6c-.8 1.4-1.2 2.95-1.2 4.5h27.5z" fill="#00832D"/>
                      <path d="M73.55 76.8L59.8 53h-27.5l13.75 23.8c1.35.8 2.9 1.2 4.4 1.2h18.5c1.55 0 3.1-.4 4.45-1.2z" fill="#2684FC"/>
                      <path d="M43.65 25h27.5c0-1.55-.4-3.1-1.2-4.5l-20-34.6c-.8-1.4-1.95-2.5-3.3-3.3z" fill="#FFBA00"/>
                    </svg>
                  </div>
                  <div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors font-medium text-sm text-slate-700"
              >
                {isConnectingDrive ? (
                  <>Connect...</>
                ) : (
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-slate-600 font-medium text-sm hover:text-slate-800 transition-colors">
            Cancel
          </button>
          <button 
            onClick={handleSave}
            className="px-6 py-2 bg-blue-600 text-white rounded-lg font-medium text-sm hover:bg-blue-700 shadow-sm flex items-center gap-2"
          >
            <Save size={16} />
            Save Changes
          </button>
